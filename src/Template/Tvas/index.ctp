...
<span class="pull-right"> <?= $this->Html->link(__(' Ajouter'),'/tvas/add/',['class' => 'btn btn-success']); ?></span>
<div class="widget-box">
  <div class="widget-title"> <span class="icon"><i class="fa fa-user"></i></span>
    <h5>Les taux de TVA</h5>
  </div>
  ...
  <?php foreach ($tvas as $tva): ?>
        <tr class="grade<?= $this->Number->format($tva->id) ?>">
          <td><?= $this->Number->format($tva->id) ?></td>
          <td><?= h($tva->taux) ?>%</td>
          <td class="actions">
              <?= $this->Html->link(__('<i class="fa fa-pencil"></i>'), ['action' => 'edit', $tva->id],['escape' => false]) ?>
              <?= $this->Form->postLink(__('<i class="fa fa-trash"></i>'), ['action' => 'delete', $tva->id], ['confirm' => __('Etes vous certain de vouloir supprimer {0}?', $tva->id) , 'escape' => false ]) ?>
          </td>
        </tr>
  <?php endforeach; ?>
